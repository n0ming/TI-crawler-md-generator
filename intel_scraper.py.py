...
header = ['AttachCheck','Profile','Url','Date','Origin_Content', 'Translated_Content','ImageUrl','AttachTweet','AttachUrl']
sub_link = False
Index = 'X'
with open(f"data_{month}_{day}.csv", mode='w', encoding='utf-8-sig', newline='') as file:
...
def save_csv(browser_url,browser):
    ...
    while True:
        ...
        with open(f"data_{month}_{day}.csv", mode='a', encoding='utf-8-sig', newline='') as file:
            writer = csv.writer(file)
...
def main():
    ...
    for url in urlsaaa:
        print(url)
        if "https" in url:  
            save_csv(url,browser)  # 최신 트윗 URL 접속 및 데이터 수집
        else:
            pass
